...
namespace CoordinatesAudit.ViewModels
{
    public sealed class AuditRowViewModel
    {
        ...
        {
            string deltas = comparison.Status == "UNAVAILABLE"
                ? "Comparison deltas: Unavailable"
                : $"Internal Origin delta: {FormatDistance(comparison.InternalHorizontalMm, comparison.InternalVerticalMm)}\nProject Base Point delta: {FormatDistance(comparison.ProjectBaseHorizontalMm, comparison.ProjectBaseVerticalMm)}\nSurvey Point delta: {FormatDistance(comparison.SurveyHorizontalMm, comparison.SurveyVerticalMm)}\nRotation delta: {comparison.RotationDegrees:0.###}°";
